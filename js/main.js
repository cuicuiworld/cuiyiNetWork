jQuery(function($) {
    ...
    //底部按钮适配功能分区
    (function(){
        var footLink = $('.footer-btn-in').children();
        var browserWidth = window.innerWidth || document.documentElement.clientWidth || document.body.clientWidth;
        
        // footLink.attr('href','javascript:;');
        footLink.on('mouseenter mouseleave', function(event) {
            event.preventDefault();
            if (event.type == 'mouseenter') {
                $(this).find('.footer-drop').show();
            };
            if (event.type == 'mouseleave') {
                $(this).find('.footer-drop').hide();
            };
        });
        if (browserWidth <1024) {
            footLink.eq(0).attr('href','http://www.ydcfo.com/CloudAccountant.apk');
            footLink.eq(1).attr('href','https://itunes.apple.com/cn/app/miao-zhang/id1143765602?mt=8');
            
        };
        $('footLink').off('mouseenter').unbind('mouseleave');
        
    })();
    var sideIcon=$('.cms-social-single').children('li');
    sideIcon.hover(function() {
